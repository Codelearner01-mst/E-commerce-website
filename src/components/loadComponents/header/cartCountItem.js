export function cartCountHTML() {
  return `
    <div
      id="cart-btn"
      role="button"
      aria-label="Shopping cart"
      class="relative inline-flex items-center justify-center sm:right-7 md:right-0 mr-6 cursor-pointer
             text-gray-800 hover:text-amber-800 transition-colors duration-300"
    >
      <!-- Cart icon — minimal line style -->
      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
           viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        <path stroke-linecap="round" stroke-linejoin="round"
              d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25
                 a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684
                 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106
                 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75
                 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
      </svg>

      <!-- Count badge -->
      <span
        id="cart-count"
        class="cart-badge"
      >0</span>
    </div>`;
}
